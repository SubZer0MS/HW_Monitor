...

{
	...
	case IRP_MJ_DEVICE_CONTROL:
		...
		{
		case IOCTL_MKTX_GET_DRIVER_VERSION:
			*(PULONG)pIrp->AssociatedIrp.SystemBuffer = MKTX_DRIVER_VERSION;
			pIrp->IoStatus.Information = 4;
			status = STATUS_SUCCESS;
			break;

		case IOCTL_MKTX_GET_REFCOUNT:
			*(PULONG)pIrp->AssociatedIrp.SystemBuffer = refCount;
			pIrp->IoStatus.Information = sizeof(refCount);
			status = STATUS_SUCCESS;
			break;

		case IOCTL_MKTX_READ_MSR:
			status = ReadMsr(
		...
		case IOCTL_MKTX_READ_PMC:
			...
			break;
		case IOCTL_MKTX_HALT:
			__halt();
			status = STATUS_SUCCESS;
			break;

		case IOCTL_MKTX_READ_IO_PORT:
		case IOCTL_MKTX_READ_IO_PORT_BYTE:
		case IOCTL_MKTX_READ_IO_PORT_WORD:
		case IOCTL_MKTX_READ_IO_PORT_DWORD:
			status = ReadIoPort(
			...
			break;
		case IOCTL_MKTX_WRITE_IO_PORT:
		case IOCTL_MKTX_WRITE_IO_PORT_BYTE:
		case IOCTL_MKTX_WRITE_IO_PORT_WORD:
		case IOCTL_MKTX_WRITE_IO_PORT_DWORD:
			status = WriteIoPort(
				pIrpStack->Parameters.DeviceIoControl.IoControlCode,
				pIrp->AssociatedIrp.SystemBuffer,
				pIrpStack->Parameters.DeviceIoControl.InputBufferLength,
		...
		case IOCTL_MKTX_READ_PCI_CONFIG:
			...
			break;
		case IOCTL_MKTX_WRITE_PCI_CONFIG:
...
WriteMsr(
	...
	__try
	{
		MKTX_WRITE_MSR_INPUT* param;
		param = (MKTX_WRITE_MSR_INPUT*)lpInBuffer;
...
WriteIoPort(
	...
	ULONG nPort;
	MKTX_WRITE_IO_PORT_INPUT* param;

	param = (MKTX_WRITE_IO_PORT_INPUT*)lpInBuffer;
	...
	case IOCTL_MKTX_WRITE_IO_PORT_BYTE:
		WRITE_PORT_UCHAR((PUCHAR)(ULONG_PTR)nPort, param->DATA.CharData);
		break;
	case IOCTL_MKTX_WRITE_IO_PORT_WORD:
		WRITE_PORT_USHORT((PUSHORT)(ULONG_PTR)nPort, param->DATA.ShortData);
		break;
	case IOCTL_MKTX_WRITE_IO_PORT_DWORD:
		WRITE_PORT_ULONG((PULONG)(ULONG_PTR)nPort, param->DATA.LongData);
		break;
	default:
		return STATUS_INVALID_PARAMETER;
		break;
...
ReadPciConfig(
	...
	ULONG* lpBytesReturned)
{
	MKTX_READ_PCI_CONFIG_INPUT *param;
	NTSTATUS status;

	if(nInBufferSize != sizeof(MKTX_READ_PCI_CONFIG_INPUT))
	{
		return STATUS_INVALID_PARAMETER;
	}
	param = (MKTX_READ_PCI_CONFIG_INPUT *)lpInBuffer;

	status = pciConfigRead(
				param->PciAddress,
				param->PciOffset,
				lpOutBuffer,
				nOutBufferSize
...
WritePciConfig(
	...
	MKTX_WRITE_PCI_CONFIG_INPUT *param;
	ULONG writeSize;

	if(nInBufferSize < offsetof(MKTX_WRITE_PCI_CONFIG_INPUT, Data))
	{
		return STATUS_INVALID_PARAMETER;
	}

	param = (MKTX_WRITE_PCI_CONFIG_INPUT *)lpInBuffer;
	writeSize = nInBufferSize - offsetof(MKTX_WRITE_PCI_CONFIG_INPUT, Data);

	*lpBytesReturned = 0;

	return pciConfigWrite(
				param->PciAddress,
				param->PciOffset,
...
pciConfigRead(
	...
	if(error == 0)
	{
		return MKTX_ERROR_PCI_BUS_NOT_EXIST;
	}
	else if(length != 2 && error == 2)
	{
		return MKTX_ERROR_PCI_NO_DEVICE;
	}
	else if(length != error)
	{
		return MKTX_ERROR_PCI_READ_CONFIG;
	}
...
pciConfigWrite(
	...
	if(error != length)
	{
		return MKTX_ERROR_PCI_WRITE_CONFIG;
	}
...
ReadMemory(
	...
	MKTX_READ_MEMORY_INPUT *param;
	ULONG	size;
	PHYSICAL_ADDRESS address;
	PVOID	maped;
	BOOLEAN	error;

	if(nInBufferSize != sizeof(MKTX_READ_MEMORY_INPUT))
	{
		return STATUS_INVALID_PARAMETER;
	}

	param = (MKTX_READ_MEMORY_INPUT *)lpInBuffer;
...
WriteMemory(
	...
	MKTX_WRITE_MEMORY_INPUT *param;
	ULONG size;
	PHYSICAL_ADDRESS address;
	PVOID	maped;
	BOOLEAN	error;

	if(nInBufferSize < offsetof(MKTX_WRITE_MEMORY_INPUT, Data))
	{
		return STATUS_INVALID_PARAMETER;
	}

	param = (MKTX_WRITE_MEMORY_INPUT *)lpInBuffer;

	size = param->UnitSize * param->Count;
	if(nInBufferSize < size + offsetof(MKTX_WRITE_MEMORY_INPUT, Data))
	{
		return STATUS_INVALID_PARAMETER;
	}
